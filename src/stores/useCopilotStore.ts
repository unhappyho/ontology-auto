...
const STEP_ANALYSIS_TEXTS: Record<number, string> = {
  ...
  4: '函数动作识别完成！从 3 份接口文档中提取 5 个函数，自动封装为 5 个原子动作，推荐 5 条本体-动作关联关系（最高置信度 95%），请逐步确认。',
  ...
  4: [
    {
      ...
      type: 'summary',
      title: '函数识别结果',
      description: '从接口文档中智能提取的函数列表',
      content: '已从 3 份接口文档提取 5 个函数：\n\n客户管理接口(customer_api_v1.0.json)：\n- 根据客户ID查询客户 (CUST_001)\n- 更新客户信息 (CUST_002)\n\n订单管理接口(order_api_v2.3.yaml)：\n- 查询订单列表 (ORD_001)\n- 创建订单 (ORD_002)\n\n产品管理接口(product_api_v1.5.json)：\n- 查询产品详情 (PROD_001)',
      stats: {
        '接口文档数': 3,
        '提取函数数': 5,
        '输入参数总数': 12
      ...
      action: 'applyRule',
      reidentifyAction: 'reidentify-functions'
    },
    {
      id: 's4-2',
      type: 'summary',
      title: '动作匹配结果',
      description: '基于函数自动封装或匹配已有动作',
      content: '已生成 5 个原子动作：\n\n客户管理：\n- 查询客户信息 (ACT_001) ← CUST_001\n- 更新客户信息 (ACT_004) ← CUST_002\n\n订单管理：\n- 查询订单列表 (ACT_002) ← ORD_001\n- 创建订单 (ACT_005) ← ORD_002\n\n产品管理：\n- 查询产品详情 (ACT_003) ← PROD_001',
      stats: {
        '原子动作数': 5,
        '自动封装': 5,
        '匹配已有动作': 0
      ...
      action: 'applyRule',
      reidentifyAction: 'reidentify-actions'
    },
    {
      id: 's4-3',
      type: 'relation',
      title: '本体关联推荐',
      description: '大模型推理本体与动作的关联关系',
      content: '推荐 5 条本体-动作关联：\n\n- 查询客户信息 → 客户(CRM/基础数据) 置信度 95%\n- 查询订单列表 → 订单(CRM/交易数据) 置信度 88%\n- 查询产品详情 → 产品(产品管理/基础数据) 置信度 92%\n- 查询订单列表 → 客户(CRM/基础数据) 置信度 72%\n- 查询客户信息 → 客户(CRM/基础数据) 置信度 100%',
      stats: {
        '推荐关联数': 5,
        '高置信度(≥90%)': 3,
        '最高置信度': '100%'
      ...
      action: 'addRelation',
      reidentifyAction: 'reidentify-associations'
    }
  ],
  5: [
    {
